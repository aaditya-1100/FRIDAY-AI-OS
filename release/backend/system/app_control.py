...
_app_discovery_lock = threading.Lock()
_index_loaded = False
_is_indexing = False
            ...
            for root, dirs, files in os.walk(str(base_dir)):
                for f in files:
                    if f.lower().endswith((".lnk", ".url", ".exe")):
                        ...
                        if base_name and base_name not in new_index:
                            new_index[base_name] = full_path
        except Exception as e:
            print(f"[APP DISCOVERY WARNING] Failed walking {base_dir}: {e}")

    # Add popular user mappings for convenience
    new_index["vscode"] = new_index.get("visual studio code", "")
    new_index["code"] = new_index.get("visual studio code", "")

    # Atomic swap for thread safety
    _APP_DISCOVERY_INDEX = new_index
    print(f"[APP DISCOVERY] Windows Native App Discovery Engine indexed {len(_APP_DISCOVERY_INDEX)} programs.")


def _bg_scan_installed_apps():
    global _is_indexing, _index_loaded
    try:
        scan_installed_apps()
        _index_loaded = True
    except Exception as e:
        print(f"[APP DISCOVERY ERROR] Background startup scanning failed: {e}")
    finally:
        with _app_discovery_lock:
            _is_indexing = False


def ensure_app_index_loaded():
    """Starts background scanning of Windows installed apps if not already started."""
    global _is_indexing, _index_loaded
    if _index_loaded:
        return
    with _app_discovery_lock:
        if _is_indexing or _index_loaded:
            return
        _is_indexing = True
        
    thread = threading.Thread(target=_bg_scan_installed_apps, daemon=True)
    thread.start()
...
def find_folder(folder_name: str):
    ...
    for base_path in SEARCH_PATHS:
        if not base_path.exists():
            continue
        try:
            # Depth 1: scan immediate subfolders
            for entry in os.scandir(base_path):
                if entry.is_dir():
                    name_lower = entry.name.lower()
                    if folder_name == name_lower or folder_name in name_lower:
                        return entry.path
            
            # Depth 2: scan direct children's immediate subfolders
            for entry in os.scandir(base_path):
                if entry.is_dir():
                    try:
                        for sub_entry in os.scandir(entry.path):
                            if sub_entry.is_dir():
                                name_lower = sub_entry.name.lower()
                                if folder_name == name_lower or folder_name in name_lower:
...
def open_app(app_name: str):
    try:
        ...
        # Clean / normalize slashes and expand variables if the path looks like a directory or file
        if "/" in app_name or "\\" in app_name or ":" in app_name or app_name.startswith("~") or app_name.startswith("%"):
            normalized_path = os.path.abspath(os.path.expandvars(os.path.expanduser(app_name)))
            if os.path.exists(normalized_path):
                print(f"[APP CONTROL] Normalized path exists locally. Opening: '{normalized_path}'")
...
def is_valid_open_target(target: str) -> bool:
    """
    Checks if a target represents a valid local application, settings URI, physical folder, 
    local file/dir, or registered workspace bookmark website. Used for ownership-based routing.
    """
    target_clean = target.lower().strip()
    if not target_clean:
        return False

    # 1. Native explorer, drive letters, special folders, settings synonyms
    if target_clean in ("explorer", "file explorer", "windows explorer", "my computer"):
        return True
    if re.match(r"^[a-z]\s*(?:drive|:)?$", target_clean):
        return True
    if target_clean in SPECIAL_FOLDERS:
        return True
    if target_clean in SETTINGS_MAP:
        return True
    if target_clean in APP_MAP:
        return True

    # 2. Local physical file/folder path existence check
    try:
        expanded = os.path.abspath(os.path.expandvars(os.path.expanduser(target_clean)))
        if os.path.exists(expanded) or os.path.exists(target):
            return True
    except Exception:
        pass

    # 3. Dynamic Windows installed app discovery check (Registry & Start Menu)
    ensure_app_index_loaded()
    if target_clean in _APP_DISCOVERY_INDEX:
        return True
    
    # Synonyms (e.g. "spotify app" -> "spotify")
    if target_clean.endswith(" app") and target_clean.replace(" app", "").strip() in _APP_DISCOVERY_INDEX:
        return True

    # Fuzzy matches
    for key, val in _APP_DISCOVERY_INDEX.items():
        if val and os.path.exists(val) and (target_clean in key or key in target_clean) and len(target_clean) >= 4:
            return True

    # Deep folder search
    if find_folder(target_clean):
        return True

    # 4. Websites check (registered in site_registry)
    from config.site_registry import get_workspace_url
    if get_workspace_url(target_clean):
        return True

    # Direct domain/URL check
    if "." in target_clean and " " not in target_clean:
        return True

    # Common canonical services check
    common_services = {"github", "reddit", "twitter", "x", "instagram", "linkedin", "facebook", "amazon", "flipkart", "netflix", "wikipedia", "whatsapp", "gmail", "drive", "maps", "translate", "canva", "figma", "stackoverflow", "stack overflow", "claude", "perplexity", "huggingface", "hugging face"}
    if any(re.search(r'\b' + re.escape(alias) + r'\b', target_clean) for alias in common_services):
        return True
